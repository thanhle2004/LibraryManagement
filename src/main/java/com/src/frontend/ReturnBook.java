...
public class ReturnBook extends JFrame {
    ...
    private JTextField issueIDField, borrowerNameField, bookNameField, issueDateField, dueDateField, returnDateField, daysOverdueField, fineAmountField;
    ...
    private void initComponents(){
        ...
        checkButton.addActionListener(e -> {
            ...
            if (bookId.isEmpty() || borrowerId.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Please enter both Book ID and Borrower ID", "Input Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            if (borrowing == null) {
                JOptionPane.showMessageDialog(this, "No active borrowing record found for this Book ID and Borrower ID", "Record Not Found", JOptionPane.ERROR_MESSAGE);
                return;
            }
        ...
        confirmButton.addActionListener(e -> {
            ...
            // Find borrowing record again for update
            BorrowingDAO borrowingDAO = new BorrowingDAO();
            Map<String, Object> borrowing = null;
            for (Map<String, Object> record : borrowingDAO.findAll()) {
                if (bookId.equals(record.get("ISBN")) &&
            ...
            if (borrowing == null) {
                JOptionPane.showMessageDialog(this, "No active borrowing record found", "Record Not Found", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            //     JOptionPane.showMessageDialog(this, "Book returned successfully! Fine: $" + String.format("%.2f", fine), "Success", JOptionPane.INFORMATION_MESSAGE);
            //     clearFields();
            //     returnPanel.remove(confirmButton);
            //     returnPanel.revalidate();
            //     returnPanel.repaint();
            // } catch (Exception ex) {
            //     JOptionPane.showMessageDialog(this, "Error returning book: " + ex.getMessage(), "Database Error", JOptionPane.ERROR_MESSAGE);
            // }

            try {
                borrowingDAO.update(borrowing); // Trigger will handle Book.Status update
                JOptionPane.showMessageDialog(this, "Book returned successfully! Fine: $" + String.format("%.2f", fine), "Success", JOptionPane.INFORMATION_MESSAGE);
                clearFields();
                returnPanel.remove(confirmButton);
                returnPanel.revalidate();
